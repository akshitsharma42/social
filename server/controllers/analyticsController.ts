...
export const getAnalyticsOverview = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        for (const metric of metrics) {
            ...
            if (!latestMetrics.has(key)) latestMetrics.set(key, metric);
        }

        const platformMap = new Map<string, { posts: number; impressions: number; reach: number; likes: number; comments: number; shares: number; clicks: number }>();
        ...
        const topPosts = posts.map((post) => {
            ...
            for (const platform of post.platforms) {
                ...
                if (metric) {
                    current.impressions += metric.impressions; current.reach += metric.reach;
                    current.likes += metric.likes; current.comments += metric.comments;
                    current.shares += metric.shares; current.clicks += metric.clicks;
                }
                platformMap.set(platform, current);
            }
            impressions += totals.impressions; reach += totals.reach; 
...
export const addPostMetric = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        if (!ownedPost) { res.status(404).json({ message: "Post not found" }); return; }
